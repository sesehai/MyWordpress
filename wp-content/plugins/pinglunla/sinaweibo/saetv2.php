 ...
 * @ignore
 */
class Pinglunla_OAuthException extends Exception {
	// pass
}
...
class Pinglunla_SaeTOAuthV2 {
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new Pinglunla_OAuthException("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new Pinglunla_OAuthException("get access token failed." . $token['error']);
		}
		return $token;
	}
...
class Pinglunla_SaeTClientV2
{
	...
	{
		$this->oauth = new Pinglunla_SaeTOAuthV2( $akey, $skey, $access_token, $refresh_token );
	}
