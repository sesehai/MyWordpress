...
class Pinglunla_OpenSDK_OAuth_Client
{
	/**
	 * 签名的url标签
	 * @var string
	...
	public function request( $url, $method, $params, $multi = false, $is_oauth=true)
	{
		if($is_oauth){ 	//使用oauth参数集调用接口
			...
			$urls = @parse_url($url);
			$sig = Pinglunla_SnsSigCheck::makeSig($method, $urls['path'], $params, $this->_app_secret.'&');
			$params['sig'] = $sig;
			print_r($params);
		}		
	...
	private function sign( $url , $method, $params )
	{
        ...
        {
			$key = Pinglunla_OpenSDK_Util::urlencode_rfc3986($key);
            if(is_array($value))
            {
                ...
                {
                    $pairs[] = $key . '=' . Pinglunla_OpenSDK_Util::urlencode_rfc3986($duplicate_value);
                }
            }
            else
            {
                $pairs[] = $key . '=' . Pinglunla_OpenSDK_Util::urlencode_rfc3986($value);
            }
        }
		
        $sign_parts = Pinglunla_OpenSDK_Util::urlencode_rfc3986(implode('&', $pairs));
		
		$base_string = implode('&', array( strtoupper($method) , Pinglunla_OpenSDK_Util::urlencode_rfc3986($url) , $sign_parts ));

        $key_parts = array(Pinglunla_OpenSDK_Util::urlencode_rfc3986($this->_app_secret), Pinglunla_OpenSDK_Util::urlencode_rfc3986($this->_token_secret));

        $key = implode('&', $key_parts);
        $sign = base64_encode(Pinglunla_OpenSDK_Util::hash_hmac('sha1', $base_string, $key, true));
		if($this->_debug)
		{
