...
class Pinglunla_OpenSDK_Tencent_Weibo extends Pinglunla_OpenSDK_OAuth_Interface
{
	...
	public static function call($command , $params=array() , $method = 'GET' ,$multi=false ,$is_oauth=true, $decode=true , $format=self::RETURN_JSON)
	{
		...
		if($decode)
		{
			...
			{
				return Pinglunla_OpenSDK_Util::json_decode($response, true);
			}
			else
			{
	...
	protected static function getOAuth()
	{
		if( null === self::$oauth )
		{
			self::$oauth = new Pinglunla_OpenSDK_OAuth_Client(self::$_appsecret);
