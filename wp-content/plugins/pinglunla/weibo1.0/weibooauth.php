 ...
 * @ignore
 */
class Pinglunla_sina_OAuthException extends Exception {
	// pass
}
...
class Pinglunla_OAuthConsumer {
	...
	function __toString() {
		return "Pinglunla_OAuthConsumer[key=$this->key,secret=$this->secret]";
	}
}
...
class Pinglunla_OAuthToken {
	...
	function to_string() {
		return "oauth_token=" .
			Pinglunla_OAuthUtil::urlencode_rfc3986($this->key) .
			"&oauth_token_secret=" .
			Pinglunla_OAuthUtil::urlencode_rfc3986($this->secret);
	}
 ...
 * @ignore
 */
class Pinglunla_OAuthSignatureMethod {
 ...
 * @ignore
 */
class Pinglunla_OAuthSignatureMethod_HMAC_SHA1 extends Pinglunla_OAuthSignatureMethod {
	function get_name() {
		return "HMAC-SHA1";
	}
	...
	public function build_signature($request, $consumer, $token) {
		...
		$key_parts = Pinglunla_OAuthUtil::urlencode_rfc3986($key_parts);
 ...
 * @ignore
 */
class Pinglunla_OAuthSignatureMethod_PLAINTEXT extends Pinglunla_OAuthSignatureMethod {
	...
	public function build_signature($request, $consumer, $token) {
		$sig = array(
			Pinglunla_OAuthUtil::urlencode_rfc3986($consumer->secret)
		);

		if ($token) {
			array_push($sig, Pinglunla_OAuthUtil::urlencode_rfc3986($token->secret));
		} else {
			array_push($sig, '');
		}
		...
		return Pinglunla_OAuthUtil::urlencode_rfc3986($raw);
	}
}
 ...
 * @ignore
 */
class Pinglunla_OAuthSignatureMethod_RSA_SHA1 extends Pinglunla_OAuthSignatureMethod {
	public function get_name() {
		return "RSA-SHA1";
	}
...
class Pinglunla_OAuthRequest {
	...
	public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) {
		...
		if (!$parameters) {
			...
			$request_headers = Pinglunla_OAuthUtil::get_headers();

			// Parse the query-string to find GET parameters
			$parameters = Pinglunla_OAuthUtil::parse_parameters($_SERVER['QUERY_STRING']);
			...
			if ($http_method == "POST"
				...
				$post_data = Pinglunla_OAuthUtil::parse_parameters(
			...
			if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") {
				$header_parameters = Pinglunla_OAuthUtil::split_header(
		...
		return new Pinglunla_OAuthRequest($http_method, $http_url, $parameters);
	}
	...
	public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL) {
		@$parameters or $parameters = array();
		$defaults = array("oauth_version" => Pinglunla_OAuthRequest::$version,
			"oauth_nonce" => Pinglunla_OAuthRequest::generate_nonce(),
			"oauth_timestamp" => Pinglunla_OAuthRequest::generate_timestamp(),
	...
	public function get_signable_parameters() {
		...
		return Pinglunla_OAuthUtil::build_http_query($params);
	}
	...
	public function get_signature_base_string() {
		...
		$parts = Pinglunla_OAuthUtil::urlencode_rfc3986($parts);
	...
	//echo "multi=" . $multi . '`';
	if( $multi )
		return Pinglunla_OAuthUtil::build_http_query_multi($this->parameters);
	else
		return Pinglunla_OAuthUtil::build_http_query($this->parameters);
	}
	...
	public function to_header() {
		...
		foreach ($this->parameters as $k => $v) {
			...
			if (is_array($v)) {
				throw new Pinglunla_sina_OAuthException('Arrays not supported in headers');
			}
			$out .= ',' .
				Pinglunla_OAuthUtil::urlencode_rfc3986($k) .
				'="' .
				Pinglunla_OAuthUtil::urlencode_rfc3986($v) .
				'"';
		}
		return $out;
	}
...
class Pinglunla_OAuthServer {
	...
	private function get_version(&$request) {
		...
		if ($version && $version != $this->version) {
			throw new Pinglunla_sina_OAuthException("OAuth version '$version' not supported");
		}
		return $version;
	}
	...
	private function get_signature_method(&$request) {
		...
		if (!in_array($signature_method,
			array_keys($this->signature_methods))) {
				throw new Pinglunla_sina_OAuthException(
	...
	private function get_consumer(&$request) {
		...
		if (!$consumer_key) {
			throw new Pinglunla_sina_OAuthException("Invalid consumer key");
		}

		$consumer = $this->data_store->lookup_consumer($consumer_key);
		if (!$consumer) {
			throw new Pinglunla_sina_OAuthException("Invalid consumer");
		}
	...
	private function get_token(&$request, $consumer, $token_type="access") {
		...
		if (!$token) {
			throw new Pinglunla_sina_OAuthException("Invalid $token_type token: $token_field");
		}
		return $token;
	}
	...
	private function check_signature(&$request, $consumer, $token) {
		...
		if (!$valid_sig) {
			throw new Pinglunla_sina_OAuthException("Invalid signature");
		}
	}
	...
	private function check_nonce($consumer, $token, $nonce, $timestamp) {
		...
		if ($found) {
			throw new Pinglunla_sina_OAuthException("Nonce already used: $nonce");
		}
	}
 ...
 * @ignore
 */
class Pinglunla_OAuthDataStore {
 ...
 * @ignore
 */
class Pinglunla_OAuthUtil {

	public static $boundary = '';

	public static function urlencode_rfc3986($input) {
		if (is_array($input)) {
			return array_map(array('Pinglunla_OAuthUtil', 'urlencode_rfc3986'), $input);
	...
	public static function split_header($header, $only_allow_oauth_parameters = true) {
		...
		while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
			...
			if (preg_match('/^oauth_/', $header_name) || !$only_allow_oauth_parameters) {
				$params[$header_name] = Pinglunla_OAuthUtil::urldecode_rfc3986($header_content);
			}
	...
	public static function parse_parameters( $input ) {
		...
		foreach ($pairs as $pair) {
			...
			$parameter = Pinglunla_OAuthUtil::urldecode_rfc3986($split[0]);
			$value = isset($split[1]) ? Pinglunla_OAuthUtil::urldecode_rfc3986($split[1]) : '';
	...
	public static function build_http_query_multi($params) {
		...
		//$keys = Pinglunla_OAuthUtil::urlencode_rfc3986(array_keys($params));
		//$values = Pinglunla_OAuthUtil::urlencode_rfc3986(array_values($params));
...
class Pinglunla_SaeTClient
{
	...
	{
		$this->oauth = new Pinglunla_SaeTOAuth( $akey , $skey , $accecss_token , $accecss_token_secret );
	}
...
class Pinglunla_SaeTOAuth {
	...
	/**
	 * construct Pinglunla_WeiboOAuth object
	 */
	function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) {
		$this->sha1_method = new Pinglunla_OAuthSignatureMethod_HMAC_SHA1();
		$this->consumer = new Pinglunla_OAuthConsumer($consumer_key, $consumer_secret);
		if (!empty($oauth_token) && !empty($oauth_token_secret)) {
			$this->token = new Pinglunla_OAuthConsumer($oauth_token, $oauth_token_secret);
		} else {
			$this->token = NULL;
		}
	}
	...
	function getRequestToken($oauth_callback = NULL) {
		...
		$token = Pinglunla_OAuthUtil::parse_parameters($request);
		$this->token = new Pinglunla_OAuthConsumer($token['oauth_token'], $token['oauth_token_secret']);
		return $token;
	}
	...
	function oAuthRequest($url, $method, $parameters , $multi = false) {
		...
		// echo $url ;
		$request = Pinglunla_OAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters);
	...
	function http($url, $method, $postfields = NULL , $multi = false) {
		...
		$header_array2=array();
		if( $multi )
			$header_array2 = array("Content-Type: multipart/form-data; boundary=" . Pinglunla_OAuthUtil::$boundary , "SaeRemoteIP: " . $_SERVER['REMOTE_ADDR'] , "Expect: ");
 ...
 * 
 */
class Pinglunla_WeiboOAuth extends Pinglunla_SaeTOAuth
{
	function __construct( $consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL )
	{
 ...
 * 
 */
class Pinglunla_WeiboClient extends Pinglunla_SaeTClient
{
	function __construct( $consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL )
	{
		parent::__construct( $consumer_key, $consumer_secret, $oauth_token , $oauth_token_secret );
